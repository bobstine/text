...
# --- effect of scaling/normalization on word regressions
	...
	w <- t( t(w) * 1/(sqrt(type.freq)) )
	predictive.r2( regr.raw <- lm(logPrice ~ poly(logTokens,5) + w) )
	
	# normalize by averaging within document after col adjustment
	w <- W[,1:k]
	type.freq <- apply(w,2,sum)
	w <- t( t(w) * 1/(sqrt(type.freq)) )
	w <- w * (1/nTokens)
	...
	# normalize by sqrt within document after col adjustment (CCA style)
	w <- W[,1:k]
	type.freq <- apply(w,2,sum)
	w <- t( t(w) * 1/(sqrt(type.freq)) )
	w <- w * (1/sqrt(nTokens))
...
# --- sequence of R2 statistics (rest done in C++)
	...
	plot((r2.len - r2.none)[1:100], ylim=c(0,0.20), ylab="Increase in R2 with len", xlab="# Words")
...
#     point is that words are more collinear than you might think
...
#     1/sqrt(nTokens):  much larger effect
	...
	udv  <- svd(www)
	# compare to R's principal components
	pca  <- princomp(www)
